<x-layouts.app title="Categorias">
    ...
    <h1>Listado de categorías. Aquí podrá agregar, y borrar categorías que no estén en uso si lo desea.</h1><br>
    @if (session('mensaje'))
        ...
        <div class="grid grid-cols-2 gap-4">
            @foreach ($categories as $category)
                <div
                    ...
                    <span
                        class="text-lg font-semibold border-2 border-transparent rounded dark:text-slate-300 text-slate-600 focus:border-slate-500 focus:outline-none">{{ $category->name }}</span>
                    <form action="{{ route('deleteColor', ['id' => $category->id]) }}" method="POST"
